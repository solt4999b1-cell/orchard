...
async function syncNow(){
  ...
  if (!db) {
    ...
    if(label) { label.textContent='Firebase 미연결'; label.style.color='#E65100'; }
    showToast('Firebase에 연결되지 않았습니다');
    return;
  }
  ...
  try {
    // 식물 데이터
    var plantsSnap = await db.collection('plants').limit(300).get();
    ...
    // 오늘 완료 작업
    var doneSnap = await db.collection('doneTasks').where('date','==',TODAY_STR).get();
    APP.doneTasks = {};
    doneSnap.docs.forEach(function(d){ APP.doneTasks[d.id]=d.data(); });

    // 기록 로드
    var wlSnap = await db.collection('workLogs').orderBy('date','desc').limit(80).get();
    var grSnap = await db.collection('growRecords').orderBy('date','desc').limit(80).get();
    APP.logs = wlSnap.docs.map(function(d){ return Object.assign({id:d.id,_col:'workLogs'},d.data()); })
      .concat(grSnap.docs.map(function(d){
        var x=d.data();
...
async function loadAllData() {
  ...
  setLoadingStep(1, 45, '로컬 데이터 완료 — Firebase 요청 중...');
  if (!db) {
    setLoadingStep(2, 100, '로컬 모드 완료');
  ...
  if (plantsSnap.docs.length === 0) {
    ...
    await seedPlants();
    try { plantsSnap = await db.collection('plants').limit(300).get(); } catch(e){ plantsSnap={docs:[]}; }
  }
  if (plantsSnap.docs.length > 0) {
    APP.plants = plantsSnap.docs.map(function(d){ return Object.assign({id:d.id}, d.data()); });
    APP.plants.sort(function(a,b){ return (a.no||0)-(b.no||0); });
  }
  APP.doneTasks = {};
  doneSnap.docs.forEach(function(d){ APP.doneTasks[d.id]=d.data(); });
  setLoadingStep(2, 70, 'Firebase 동기화 완료!');
  try { renderToday(); renderPlants(); } catch(e){}
  setSyncStatus(true);
  setTimeout(function(){ hideLoading(); }, 300);
  Promise.all([
    db.collection('workLogs').orderBy('date','desc').limit(80).get().catch(function(){ return {docs:[]}; }),
    db.collection('growRecords').orderBy('date','desc').limit(80).get().catch(function(){ return {docs:[]}; }),
    loadUserDb(),
  ]).then(function(results2) {
    var wlSnap=results2[0], grSnap=results2[1];
    var wlRows=wlSnap.docs.map(function(d){ return Object.assign({id:d.id,_col:'workLogs'},d.data()); });
...
async function loadMyPesticideList() {
  if (!db) return;
  try {
    var snap = await db.collection('myPesticides').orderBy('registeredAt','desc').get();
    window._myPesticideList = [];
    var seen = {};
    snap.forEach(function(doc){
      var d = doc.data(); d._id = doc.id; d.id = doc.id;
      // 이름 중복 제거 (첫번째만 유지)
      var nm = (d.name||'').trim().replace(/\s/g,'');
      if (nm && seen[nm]) return;
      if (nm) seen[nm] = true;
      // pest_status 기본값
      if (!d.pest_status) d.pest_status = 'have';
      window._myPesticideList.push(d);
    });
    // USER_DB['pest']와 동기화 (관리 메뉴, PSIS 우선순위에서 사용)
    USER_DB['pest'] = window._myPesticideList.map(function(p){
      return Object.assign({}, p, {_col:'myPesticides'});
    });
  } catch(e) { window._myPesticideList = []; USER_DB['pest'] = []; }
}
