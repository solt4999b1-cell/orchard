...
var db = {
  collection: function(colName) {
    return {
      ...
      add: async function() { return { id: 'mock_' + Date.now() }; },
...
// APP 전역 객체에 추가할 초기화 (기존 APP.plants 등은 유지)
const APP_GAS = {
  plants: [],
  workLogs: [],
  checkedTasks: [],
...
async function loadPesticidesFromGAS() {
  try {
    ...
    if (Array.isArray(response)) {
      ...
      // 다른 배열 형식
      pesticides = response;
      console.log(`[loadPesticidesFromGAS] 다른 배열: ${pesticides.length}개`);
    }
    ...
    if (pesticides && pesticides.length > 0) {
      ...
      // 농약 목록 로깅
      const pestNames = pesticides
        .map(p => p.name || p['농약명'] || p['이름'] || Object.values(p)[1])
        .filter(Boolean)
        .join(', ');
      ...
      APP.pesticides = [];
      console.warn('[loadPesticidesFromGAS] 농약 데이터 없음 - myPesticides 시트 확인 필요');
    }
...
/**
 * GAS의 raw plant 객체를 정규화
 */
function normalizeGASPlant(plant) {
  if (!plant || typeof plant !== 'object') {
    return null;
  }

  // plantDate 정규화
  let plantDate = plant.plantDate || '';
  if (plantDate) {
    if (typeof plantDate === 'string') {
      // "2026/07/15" → "2026-07-15"
      plantDate = plantDate.replace(/\//g, '-');
      // 추가 검증
      if (!/^\d{4}-\d{2}-\d{2}$/.test(plantDate)) {
        plantDate = '';
      }
    } else if (typeof plantDate === 'number') {
      // Google Sheets의 시리얼 숫자 → 날짜
      plantDate = sheetsSerialToDate(plantDate);
    }
  }

  // crops 배열 처리
  let crops = plant.crops || [];
  ...
  const validStatuses = ['active', 'harvest', 'archived'];
  const finalStatus = validStatuses.includes(status) ? status : 'active';

  // category 정규화 (없으면 name으로 유추)
  let category = plant.category || '';
  if (!category) {
    category = inferCategory(plant.name);
  }

  // fruitDays 정규화
  let fruitDays = parseInt(plant.fruitDays) || 0;

  return {
    id: plant.id || `plant_${Date.now()}_${Math.random()}`,
    ...
    crops: crops,
    fruitDays: fruitDays,
    quantity: parseInt(plant.quantity) || 0,
...
function inferCategory(name) {
  ...
  // 유실수 키워드
  const fruits = ['apple', '사과', '배', 'pear', '포도', 'grape', '딸기', 'berry', 
                   '귤', 'orange', '복숭아', 'peach', '수박', 'watermelon', '메론', 'melon'];
  if (fruits.some(f => lower.includes(f))) return '유실수';

  // 채소 키워드
  const veggies = ['tomato', '토마토', 'cucumber', '오이', '고추', 'pepper', 
                    '가지', 'eggplant', '당근', 'carrot', '무', 'radish', 
                    '배추', 'cabbage', '상추', 'lettuce', '양파', 'onion'];
  if (veggies.some(v => lower.includes(v))) return '채소';
...
function deduplicatePlants(plants) {
  ...
  for (const plant of plants) {
    ...
    if (seen.has(key)) {
      // 기존 항목이 있으면, 더 최신 updatedAt을 가진 것을 유지
      const existing = seen.get(key);
      if (new Date(plant.updatedAt) > new Date(existing.updatedAt)) {
        seen.set(key, plant);
      }
    } else {
      seen.set(key, plant);
  ...
  console.log(`[deduplicatePlants] ${plants.length} → ${unique.length}개`);
  return unique;
}
...
function normalizeDate(dateStr) {
  ...
  if (typeof dateStr === 'number') {
    // Google Sheets 시리얼 숫자
    return sheetsSerialToDate(dateStr);
  }
...
/**
 * Google Sheets 시리얼 숫자를 YYYY-MM-DD 문자열로 변환
 * (Google Sheets의 DATE 함수는 1900-01-01을 0으로 취급)
 */
function sheetsSerialToDate(serial) {
  if (!serial || typeof serial !== 'number') return '';

  const epochDate = new Date(1900, 0, 1);  // 1900-01-01
  const date = new Date(epochDate.getTime() + (serial - 1) * 24 * 60 * 60 * 1000);
...
function analyzeLoadedData() {
  console.log('[initGAS] 데이터 분석:');
  ...
  if (APP.plants.length > 0) {
    ...
    for (const plant of APP.plants) {
      // status 분포
      byStatus[plant.status] = (byStatus[plant.status] || 0) + 1;

      // category 분포
      byCategory[plant.category] = (byCategory[plant.category] || 0) + 1;

      // plantDate 있는 식물
      if (plant.plantDate) withPlantDate++;
    }

    console.log(`[initGAS] 전체 식물: ${APP.plants.length}`);
    console.log(`[initGAS] status 분포:`, byStatus);
    console.log(`[initGAS] category 분포:`, byCategory);
    console.log(`[initGAS] plantDate 있는 식물: ${withPlantDate}/${APP.plants.length}`);
  } else {
    console.warn('[initGAS] ⚠️ 식물 데이터가 없음');
  }

  // WorkLogs 분석
  console.log(`[initGAS] 작업 로그: ${APP.workLogs.length}개`);

  // CheckedTasks 분석
  console.log(`[initGAS] 완료된 할일: ${APP.checkedTasks.length}개`);

  // Pesticides 분석
  if (APP.pesticides.length > 0) {
    console.log(`[initGAS] 농약: ${APP.pesticides.length}개`, 
      APP.pesticides.map(p => p.name).join(', '));
  }
}
...
function showErrorDialog(title, message) {
  ...
  `;
  alertDiv.innerHTML = `
    <div style="font-weight: bold; color: #d32f2f; margin-bottom: 10px;">${title}</div>
...
// launchApp() 함수는 기존 그대로 사용
// renderPlants(), renderToday(), calcTodayTasks() 등도 그대로 사용
// APP.plants에 이미 모든 필터링 필요한 정보가 포함되어 있음


function startApp() { initGAS(); }


// Firestore 내부 연결 오류 메시지 억제 (앱 동작에 영향 없음)
(function() {
...
async function seedPlants() {
  // plants는 GAS에서 로드됨 (APP.plants 사용)
...
function calcTodayTasks() {
  ...
  APP.plants.forEach(function(plant){
    ...
    [].forEach(function(spray){
      ...
      var cleanPestName = (spray.pesticide || '').trim();
      var hasIt = !!myPesticideMap[cleanPestName];

      if (!sprayMap[cleanPestName]) {
        sprayMap[cleanPestName] = {
          pesticide: cleanPestName, pestType: spray.pestType,
      ...
      var alreadyExists = sprayMap[cleanPestName].plants.some(function(p){ return p.id === plant.id; });
      if (!alreadyExists) {
        sprayMap[cleanPestName].plants.push({
    ...
    [].forEach(function(fert){
      ...
      var cleanFertName = (fert.fertilizer || '').trim();
      if (!fertMap[cleanFertName]) {
        fertMap[cleanFertName] = {
          fertilizer: cleanFertName, desc: fert.desc,
          interval: fert.interval, amount: fert.amount,
          plants: []
        };
      }
      
      var alreadyExistsFert = fertMap[cleanFertName].plants.some(function(p){ return p.id === plant.id; });
      if (!alreadyExistsFert) {
        fertMap[cleanFertName].plants.push({
          id: plant.id, name: nm, emoji: plant.emoji||'🌱',
          location: plant.location||'', dfp: dfp, daysSince: daysSince,
          urgent: false
        });
...
function renderPlants() {
  ...
  function isFruitPlant(p) {
    ...
    // 2) category 없을 때: 에서 확인
    var _mP = (APP.plants || []).find(function(m){ return m.name===p.name||m.id===p.id; });
    if (_mP && _mP.category) {
  ...
  (APP.plants||[]).forEach(function(p) {
    ...
    // 🔥 수정된 부분: 이름 대신 고유 ID를 우선으로 사용하여 같은 이름의 작물 증발 방지
    var k   = p.id || p.name.trim(); 
...
function plantCardHTML(p) {
  ...
  // 🔥 작업 기록(APP.logs)에 '수확'이 있는지 먼저 확인
  ...
  // 🔥 수확 완료 시 진행률 100%로 고정 및 색상 변경
  if (isHarvested) pct = 100;
  var barCls       = isHarvested ? 'bar-late' : (pct<40?'bar-early':pct<75?'bar-mid':'bar-late');
  ...
  var progressBar = total > 0 ? '<div class="plant-bar-wrap" style="margin-top:5px;"><div class="plant-bar ' + barCls + '" style="width:' + pct + '%;"></div></div>'
    + '<div class="plant-progress-label"><span>' + (isHarvested ? '🎉 수확 종료' : pct + '% 진행') + '</span><span>' + (isHarvested ? '-' : dfp + '/' + total + '일') + '</span></div>' : '';
...
async function saveOcrResult() {
  ...
  try {
    ...
    if (typeof firebase !== 'undefined' && firebase.firestore && firebase.firestore.FieldValue) {
      saveData.serverTs = firebase.firestore.FieldValue.serverTimestamp();
    }

    
    var result = await smartSave('userDb_' + tab, saveData);

    
    saveData.id = result.id || result.localId || Date.now().toString();
    if (!USER_DB[tab]) USER_DB[tab] = [];
    USER_DB[tab].push(saveData);

    
    var savedMsg = result.source === 'local'
      ? '💾 ' + name + ' 기기에 저장 (온라인 시 자동 동기화)'
      : '✅ ' + name + ' Firebase 저장 완료';
    showToast(savedMsg);
    ...
    showToast('⚠️ 저장 실패: ' + (err.message || '알 수 없는 오류') + ' — 다시 시도하거나 관리자에 문의하세요.');
...
function getMergedDb(tab) {
  var base=[];
  if(tab==='pest') base=[].map(function(x){ return Object.assign({},x,{_src:'base'}); });
  else if(tab==='fert') base=[].map(function(x){ return Object.assign({},x,{_src:'base'}); });
...
function setSyncStatus(online){
  ...
  if (online && db) {
    ...
    label.textContent = 'Firebase 미연결';
  ...
  try {
    ...
    if (plantsArr.length > 0) {
      ...
      APP.plants = _deduped.filter(function(p) { return p; }).map(function(p) {
        ...
        if (!p.category) {
          var _mp = (APP.plants || []).find(function(m){
...
async function loadAllData() {
  ...
  if (APP.plants.length === 0) {
    // APP.plants는 GAS에서 initGAS()에서 이미 로드됨
...
function getAllDbItems() {
  return (APP.pesticides||[]).map(function(x){ return Object.assign({},x,{tab:'pest'}); })
    .concat((APP.fertilizers||[]).map(function(x){ return Object.assign({},x,{tab:'fert'}); }))
...
async function enrichItemWithAI(item) {
  ...
  var pestM = (APP.pesticides||[]).find(function(p){
...
async function syncPendingToFirebase() {
  if (_syncInProgress || !_isOnline || !db) return;
  ...
  for (var i=0; i<pending.length; i++) {
    ...
    try {
      ...
      var ref = await db.collection(item.collection).add(dataToSave);
  ...
  updatePendingBadge();
  if (success > 0) {
    showToast('✅ '+success+'건 Firebase 동기화 완료'+(fail>0?' (실패:'+fail+'건)':''));
    await loadUserDb();
    renderDb();
  }
}
...
function getRecommendedPesticides(cropName, diseaseOrPest) {
  ...
  active.forEach(function(p){
    ...
    if (masterMatch) results.push({pesticide:p, entries:[masterMatch], db:''});
  });
  return results;
}
